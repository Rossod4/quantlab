...
net CAGR 43.10%, Sharpe 5.15, max drawdown 0.00% - verdict ELIGIBLE_FOR_PAPER.
...
- This walk-forward's Sharpe RANKING across weight choices has NOT been checked against the real, netted-cost blend backtest's own ranking (walk_forward.py blends each child's NET RETURNS; strategies/blend.py blends TARGET WEIGHTS and nets costs on the already-blended book - the two disagree whenever turnover/borrow are nonlinear in weights). Read this result as 'is the weight choice stable', not as a stand-in for the netted book's own net_returns.
- Whether any CHOSEN walk-forward step had a NaN training Sharpe (which would lock in the first grid weight regardless of the others - see walk_forward.py's module docstring) cannot be checked from a WalkForwardResult alone: per-step training Sharpes are not retained on the object. Left as an open item for whoever runs the walk-forward experiment to verify independently.
...
Capacity: spread percentiles: p10=200.0bps, p25=200.0bps, p50=200.0bps, p75=200.0bps, p90=200.0bps, p99=200.0bps  |  AUM ceiling range: $2,000,000,000 - $4,000,000,000 across 4 liquidity assumption(s), 3 ticker(s). For scale only, NOT the output of this run: the old repo capacity estimate on real 2012-2026 S&P 500 data was $95,000,000-$335,000,000 AUM, under an assumed 50-name book (1/50 position_frac).
...
| no_cliff_score | soft | 1.0000 | 0.5000 | PASS | no_cliff_score=1.0000 against 0.50 (neighbourhood_size=3, neighbourhood_truncated=False, nan_points=0) - a RELATIVE-SPREAD statistic only, never evidence of quality by itself (see min_net_sharpe below). |
| min_net_sharpe | soft | 5.1527 | 0.3000 | PASS | net Sharpe 5.15 against 0.30 - gated ALONGSIDE no_cliff_score per the M05 carried item (a flat-but-bad neighbourhood must not pass on no_cliff_score alone). Sensitivity grid: (neighbourhood_size=3, neighbourhood_truncated=False, nan_points=0). |
...
| capacity_ceiling | soft | 2000000.0000 | 100.0000 | PASS | worst-case capacity ceiling is 2000000x the resolved intended capital ($1,000, configs/validation.yaml intended_capital_usd) against the 100x bar. NOTE: the capacity gate is trivially passable at this stake (2000000x against a 100x bar) - this is not evidence of edge, only that the resolved intended capital is small relative to the instrument's liquidity. |
...
| walk_forward_stability | soft | 1.0000 | 0.5000 | PASS | walk-forward chose the modal weight tuple in 100% of steps against the 50% bar. |
...
no_cliff_score = **1.0000** (neighbourhood_size=3, neighbourhood_truncated=False, nan_points=0) - shown ONLY beside min_net_sharpe below.

no_cliff_score=1.0000 against 0.50 (neighbourhood_size=3, neighbourhood_truncated=False, nan_points=0) - a RELATIVE-SPREAD statistic only, never evidence of quality by itself (see min_net_sharpe below).

net Sharpe 5.15 against 0.30 - gated ALONGSIDE no_cliff_score per the M05 carried item (a flat-but-bad neighbourhood must not pass on no_cliff_score alone). Sensitivity grid: (neighbourhood_size=3, neighbourhood_truncated=False, nan_points=0).

Base point: **lookback_months=12**

| Grid point | Net Sharpe |
|---|---|
| lookback_months=12 (base point) | 10.00 |
| lookback_months=15 | 10.00 |
| lookback_months=9 | 10.00 |
...
walk-forward chose the modal weight tuple in 100% of steps against the 50% bar.

#### Chosen weight per step

| Step date | momentum | value |
|---|---|---|
| 2008-03-31 | 0.25 | 0.75 |
| 2009-03-31 | 0.25 | 0.75 |
| 2010-03-31 | 0.25 | 0.75 |
| 2011-03-31 | 0.25 | 0.75 |
| 2012-03-31 | 0.25 | 0.75 |
| 2013-03-31 | 0.25 | 0.75 |
| 2014-03-31 | 0.25 | 0.75 |
| 2015-03-31 | 0.25 | 0.75 |
| 2016-03-31 | 0.25 | 0.75 |
| 2017-03-31 | 0.25 | 0.75 |
| 2018-03-31 | 0.25 | 0.75 |
| 2019-03-31 | 0.25 | 0.75 |
| 2020-03-31 | 0.25 | 0.75 |
![Walk-forward chosen weights](walk_forward_weights.png)

ported convention: each window's first return is omitted from CAGR and hidden from drawdown (M05 carried item; frozen under CLAUDE.md invariant #4 - see validation/rolling.py's module docstring).

| Grid point | CAGR | Vol | Sharpe | Max DD |
|---|---|---|---|---|
| Fixed 0% momentum/100% value | 3.51% | 2.30% | 1.52 | -1.00% |
| Fixed 100% momentum/0% value | 1.73% | 4.16% | 0.48 | -2.00% |
| Fixed 25% momentum/75% value | 3.09% | 1.36% | 2.29 | -0.25% |
| Fixed 50% momentum/50% value | 2.66% | 1.65% | 1.67 | -0.25% |
| Fixed 75% momentum/25% value | 2.21% | 2.81% | 0.85 | -1.13% |
| Walk-Forward | 3.09% | 1.36% | 2.29 | -0.25% |
...
| | |
|---|---|
| Execution mode | next_open - next_open = next session's open; returns are measured open-to-open on an adjusted basis, one session after the decision date. |
...
| Strategy params | `{}` |
| Backtest config | `{'abort_on_unscoreable': True, 'borrow_fee_annual_bps': 30.0, 'corwin_schultz_lookback_days': 60, 'cost_model': 'flat_bps', 'delisting_haircut': 0.0, 'end': '2019-12-31', 'execution': 'next_open', 'extreme_return_bound': 3.0, 'extreme_return_policy': 'exclude_legacy', 'initial_capital': 1000000.0, 'max_dropped_fraction': 0.05, 'one_way_cost_bps': 10.0, 'rebalance_freq': 'month_end', 'start': '2015-01-31'}` |
| Providers | `{}` |
